...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO customers (customer_name, phone_number, email) VALUES (?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$customer_name, $phone, $email]);
...
<html>
<head>
    <title>Add Customer - DukaSmart RMS</title>
    <style>
        /* Copy the same styles as add_product.php */
        * {
            margin: 0;
            padding: 0;
